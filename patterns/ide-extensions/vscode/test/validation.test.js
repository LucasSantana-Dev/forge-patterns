...
describe('validation', () => {
    ...
    describe('ESLint config check', () => {
        it('passes when eslint.config.js exists', async () => {
            ...
            const results = await (0, validation_1.validateCompliance)(tmpDir);
            const eslint = results.filter(r => r.message.includes('ESLint'));
            expect(eslint.length).toBe(0);
        });
        it('warns when no ESLint config found', async () => {
            const results = await (0, validation_1.validateCompliance)(tmpDir);
            const eslint = results.find(r => r.message.includes('ESLint'));
    ...
    describe('Prettier config check', () => {
        it('passes when .prettierrc exists', async () => {
            ...
            const results = await (0, validation_1.validateCompliance)(tmpDir);
            const prettier = results.filter(r => r.message.includes('Prettier'));
            expect(prettier.length).toBe(0);
        });
        it('warns when no Prettier config found', async () => {
            const results = await (0, validation_1.validateCompliance)(tmpDir);
            const prettier = results.find(r => r.message.includes('Prettier'));
    ...
    describe('TypeScript strict check', () => {
        it('passes when strict is true', async () => {
            ...
            const results = await (0, validation_1.validateCompliance)(tmpDir);
            const ts = results.filter(r => r.message.includes('strict mode'));
        ...
        it('warns when strict is false', async () => {
            ...
            const results = await (0, validation_1.validateCompliance)(tmpDir);
            const ts = results.find(r => r.message.includes('strict mode'));
    ...
    describe('package.json scripts check', () => {
        it('passes when lint, format, test exist', async () => {
            ...
            const results = await (0, validation_1.validateCompliance)(tmpDir);
            const scripts = results.filter(r => r.message.includes('script'));
    ...
    describe('secret scanning', () => {
        it('detects hardcoded passwords', async () => {
            fs.writeFileSync(path.join(tmpDir, 'config.ts'), 'const password = "hunter2";');
            const results = await (0, validation_1.validateCompliance)(tmpDir);
            const secrets = results.filter(r => r.rule === 'BR-001 Zero-Secrets');
            expect(secrets.length).toBeGreaterThan(0);
            expect(secrets[0].severity).toBe('error');
            expect(secrets[0].line).toBe(1);
        });
        it('detects API keys', async () => {
            fs.writeFileSync(path.join(tmpDir, 'env.ts'), 'const API_KEY = "sk-abc123";');
            const results = await (0, validation_1.validateCompliance)(tmpDir);
            const secrets = results.filter(r => r.rule === 'BR-001 Zero-Secrets');
            expect(secrets.length).toBeGreaterThan(0);
        });
        it('ignores placeholder patterns', async () => {
            fs.writeFileSync(path.join(tmpDir, 'config.ts'), 'const password = "{{REPLACE_WITH_PASSWORD}}";');
            const results = await (0, validation_1.validateCompliance)(tmpDir);
            const secrets = results.filter(r => r.rule === 'BR-001 Zero-Secrets');
        ...
        it('skips node_modules', async () => {
            ...
            fs.writeFileSync(path.join(tmpDir, 'node_modules', 'lib.ts'), 'const secret = "leaked";');
            const results = await (0, validation_1.validateCompliance)(tmpDir);
            const secrets = results.filter(r => r.rule === 'BR-001 Zero-Secrets');
            expect(secrets.length).toBe(0);
        });
        it('skips non-text file extensions', async () => {
            fs.writeFileSync(path.join(tmpDir, 'image.png'), 'fake binary');
            const results = await (0, validation_1.validateCompliance)(tmpDir);
            const secrets = results.filter(r => r.rule === 'BR-001 Zero-Secrets');
            expect(secrets.length).toBe(0);
        });
        it('detects secrets in nested directories', async () => {
            ...
            fs.writeFileSync(path.join(tmpDir, 'src', 'config', 'db.ts'), 'const token = "abc123";');
            const results = await (0, validation_1.validateCompliance)(tmpDir);
            const secrets = results.filter(r => r.rule === 'BR-001 Zero-Secrets');
            expect(secrets.length).toBeGreaterThan(0);
        });
  ...
  "compilerOptions": {
    ...
    describe('publishDiagnostics', () => {
        it('publishes results to diagnostic collection', () => {
            ...
            (0, validation_1.publishDiagnostics)(collection, tmpDir, results);
        ...
        it('handles empty results', () => {
            ...
            (0, validation_1.publishDiagnostics)(collection, tmpDir, []);
            expect(collection.clear).toHaveBeenCalled();
