...
export class QualityCollector extends BaseCollector {
  ...
  protected async doCollect(
    ...
    const lintPassed = context['lint_passed'] as
      | boolean
      | undefined;
    ...
    const typeCheckPassed = context['type_check_passed'] as
      | boolean
      | undefined;
    breakdown['type_check'] =
      typeCheckPassed === false ? 0 : 100;
    if (typeCheckPassed === false) {
      violations.push('TypeScript type check failed');
    }
    total += breakdown['type_check'];

    const coverage = (context['coverage_percent'] as
      | number
      | undefined) ?? 0;
    breakdown['coverage'] = Math.min(100, coverage);
    if (coverage < 80) {
      violations.push(
        `Test coverage ${coverage}% is below 80% threshold`
      );
    }
    total += breakdown['coverage'];

    const hasTests = context['has_tests'] as
      | boolean
      | undefined;
    ...
    const a11yPassed = context['accessibility_passed'] as
      | boolean
      | undefined;
    breakdown['accessibility'] =
      a11yPassed === false ? 0 : 100;
    if (a11yPassed === false) {
