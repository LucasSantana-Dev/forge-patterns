...
export class SecurityCollector extends BaseCollector {
  ...
  protected async doCollect(context: Record<string, unknown>): Promise<CollectorResult> {
    ...
    const secretsScan = context['secrets_scan_passed'] as boolean | undefined;
    ...
    const depVulns = (context['dependency_vulnerabilities'] as number | undefined) ?? 0;
    breakdown['dependencies'] = depVulns === 0 ? 100 : Math.max(0, 100 - depVulns * 10);
    if (depVulns > 0) {
      violations.push(`${depVulns} dependency vulnerabilities found`);
    }
    total += breakdown['dependencies'];

    const authEnabled = context['auth_enabled'] as boolean | undefined;
