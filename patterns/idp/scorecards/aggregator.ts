...
export class ScorecardAggregator {
  ...
  async aggregate(
    ...
    for (const { name, result } of results) {
      ...
      const w =
        (this.weights as Record<string, number>)[name] ?? 0.1;
      weightedTotal += result.score * w;
      weightSum += w;
    }

    const overallScore =
      weightSum > 0 ? Math.round(weightedTotal / weightSum) : 0;

    const allViolations = results.flatMap(
      ({ name, result }) =>
        result.violations.map((v) => `[${name}] ${v}`)
    );

    const recommendations =
      this.generateRecommendations(allViolations);
  ...
  private generateRecommendations(
    ...
    if (violations.some((v) => v.includes('secret'))) {
      recs.push(
        'Enable secret scanning in CI and remove exposed credentials'
      );
    }
    ...
    if (violations.some((v) => v.includes('RLS') || v.includes('rls'))) {
      recs.push(
        'Enable row-level security on all user-facing database tables'
      );
    }
