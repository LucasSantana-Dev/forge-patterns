...
const REACT_RULES: TemplateRule[] = [
  {
    ...
    conditions: [{ field: 'quality.a11y_passed', operator: 'eq', value: false }],
    actions: [{ type: 'warn', message: 'Components must pass accessibility checks' }],
    enabled: true
  },
  {
    id: 'react-002',
    ...
    actions: [{ type: 'block', message: 'Component test coverage below 70%' }],
    enabled: true
  }
];
...
const NEXTJS_RULES: TemplateRule[] = [
  ...
  })),
  {
    ...
    conditions: [{ field: 'performance.bundle_size_kb', operator: 'gt', value: 300 }],
  ...
  },
  {
    ...
    description: 'Warn if client components exceed 40% of total components',
    conditions: [
      {
...
const NODE_RULES: TemplateRule[] = [
  ...
  },
  {
    ...
    actions: [{ type: 'warn', message: 'Unused dependencies detected' }],
    enabled: true
  },
  {
    id: 'node-003',
...
const TEMPLATES: Record<TemplateName, TemplateConfig> = {
  ...
  nextjs: {
    ...
    ciTriggers: ['app/**/*.tsx', 'src/**/*.tsx', 'next.config.*']
  },
  node: {
