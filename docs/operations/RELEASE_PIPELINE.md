...
This document describes the automated release pipeline for the @forgespace/core package.
...
```bash
# Create a new release branch for version 1.2.0
...
Create a pull request from `release/1.2.0` to `main`:
...
2. **Run Quality Checks**: Execute full test suite, linting, and security scans
3. **Update Version**: Bump package.json to the release version
4. **Update CHANGELOG**: Add release notes to CHANGELOG.md
5. **Publish to npm**: Publish the package with provenance
6. **Create GitHub Release**: Generate a tagged GitHub release
7. **Notify Teams**: Send success/failure notifications
...
Examples:
- `release/1.2.0` - Minor release with new features
- `release/1.1.5` - Patch release with bug fixes
- `release/2.0.0` - Major release with breaking changes
...
1. **Tag Directly**: Create and push a tag directly:
   ```bash
   git tag v1.1.6
   git push origin v1.1.6
   ```

2. **Manual Publishing**: Use the existing publish workflow:
   ```bash
   npm version 1.1.6
   npm publish
   ```
...
- **UIForge MCP**: Integration testing with forge-context tools
- **UIForge WebApp**: Frontend compatibility verification
- **Documentation Sites**: Automatic documentation updates
