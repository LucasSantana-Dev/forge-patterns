...
This document maps available skills to prompt categories and use cases, enabling
intelligent prompt selection and enhancement across the Forge ecosystem.
...
function selectOptimalSkills(project, task, context) {
  ...
  return prioritizeSkills([...baseSkills, ...taskSkills, ...contextSkills]);
}
...
- Highest impact skills: Frontend Design, Brainstorming, Vercel Composition
  Patterns
- Project-specific favorites: Supabase Postgres (WebApp), MCP Docs Search (MCP
  Gateway)
...
This skills integration framework enables the creation of intelligent,
context-aware prompts that leverage the full power of available skills while
maintaining project-specific relevance and effectiveness.
