...
Skill-enhanced prompt templates for MCP Gateway development, integrating
systematic debugging, MCP docs search, and brainstorming capabilities.
...
You are a Docker specialist working on the MCP Gateway. The [service] is
experiencing [performance/resource issues].
...
DOCKER SPECIFICS: ✅ CONTAINER ANALYSIS:
...
You are a FastAPI specialist working on the MCP Gateway. The [endpoint/service]
is experiencing [error/issue].
...
FASTAPI SPECIFICS: ✅ ENDPOINT ANALYSIS:
...
You are working on the MCP Gateway AI router. The [routing/selection] system
needs [optimization/enhancement].
...
ROUTER SPECIFICS: ✅ SELECTION LOGIC:
...
You are enhancing the MCP Gateway tool selection system. The
[selection/matching] process needs [improvement/optimization].
...
SELECTION SPECIFICS: ✅ ALGORITHM ANALYSIS:
...
You are working on the MCP Gateway service manager. The [sleep/wake/lifecycle]
system needs [enhancement/optimization].
...
LIFECYCLE SPECIFICS: ✅ DOCKER INTEGRATION:
...
You are optimizing the MCP Gateway performance. The [metric/component] shows
...
PERFORMANCE SPECIFICS: ✅ METRICS ANALYSIS:
...
You are troubleshooting the MCP Gateway. The [component/service] is experiencing
[specific issue/error].
...
MCP GATEWAY SPECIFICS: ✅ COMPONENT ANALYSIS:
...
You are enhancing the MCP Gateway deployment automation. The [deployment/CI/CD]
process needs [improvement/optimization].
...
DEPLOYMENT SPECIFICS: ✅ AUTOMATION ENHANCEMENT:
...
1. **Select Appropriate Template**: Choose the template that matches your task
   type
2. **Apply Systematic Debugging**: Always use the systematic debugging
   methodology
3. **Leverage MCP Tools**: Use @[/get-context], @[/skill-mcp-docs-search], and
   @[/mcp-docs-and-tools]
4. **Follow Best Practices**: Adhere to Docker, FastAPI, and Python best
   practices

### **For Troubleshooting**

1. **Use Systematic Approach**: Follow the 5-step systematic debugging
   methodology
...
These enhanced prompts leverage systematic debugging, brainstorming, and
MCP-specific knowledge to provide comprehensive, context-aware guidance for MCP
Gateway development and maintenance.
