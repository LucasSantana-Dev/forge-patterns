...
description:
  Testing strategy and quality gates. Apply when creating or updating tests, or
...
**When to apply:** Writing or modifying test suites, quality gates, or coverage
targets across all Forge Space projects.
...
- **E2E**: 100% coverage for critical user flows (auth, project CRUD,
  generation)
...
- Integration tests: `src/__tests__/integration/my-flow.test.ts` or
  `*.integration.test.ts`
...
4. **Do NOT mix test runner imports** — use globals available for the configured
   runner
...
4. **Tests**: `pytest tests/ -v --cov=src` — 100% pass rate, coverage threshold
   met
